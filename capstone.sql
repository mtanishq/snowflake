...
CREATE MASKING POLICY GLOBALBANK.RAW_DB.EMAIL_MASK AS
(EMAIL VARCHAR) RETURNS VARCHAR ->
CASE WHEN CURRENT_ROLE = 'ADMIN' THEN EMAIL
ELSE REGEXP_REPLACE(EMAIL, '.+\@', '*****@')
END;
ALTER TABLE GLOBALBANK.RAW_DB.RAW_CUSTOMER MODIFY COLUMN email SET MASKING POLICY GLOBALBANK.RAW_DB.EMAIL_MASK;
CREATE MASKING POLICY GLOBALBANK.RAW_DB.Phone_MASK AS
(PHONE VARCHAR) RETURNS VARCHAR ->
CASE WHEN CURRENT_ROLE = 'ADMIN' THEN PHONE
ELSE SUBSTR(PHONE, 0, 5) || '***-****'
END;
ALTER TABLE GLOBALBANK.RAW_DB.RAW_CUSTOMER MODIFY COLUMN phone_number SET MASKING POLICY GLOBALBANK.RAW_DB.Phone_MASK;
CREATE OR REPLACE MASKING POLICY GLOBALBANK.RAW_DB.customer_id_MASK AS
(Cust_id VARCHAR) RETURNS VARCHAR ->
CASE
WHEN CURRENT_ROLE() = 'ADMIN' THEN Cust_id
ELSE 'XXXXXX'
END;
ALTER TABLE GLOBALBANK.RAW_DB.RAW_CUSTOMER MODIFY COLUMN phone_number SET MASKING POLICY GLOBALBANK.RAW_DB.customer_id_MASK;